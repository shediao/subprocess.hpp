# run_coverage.cmake
# Executed via: cmake -D... -P run_coverage.cmake
# Expects these -D variables:
#   COVERAGE_BINARY_DIR  - CMake binary directory
#   ALL_TEST_BINARY      - path to the all_test executable
...
# Use an absolute path so profiles land in the right place regardless of
# each test's working directory (ctest sets CWD = test executable dir).
...
if(profraw_count EQUAL 0)
  message(FATAL_ERROR
    "No .profraw files found under ${COVERAGE_DIR}. "
...
if(NOT merge_result EQUAL 0)
  message(FATAL_ERROR "llvm-profdata merge failed (exit ${merge_result}):\n${merge_error}")
endif()
...
# Regex to exclude test infrastructure from coverage reports.
# Keeps only project source (subprocess.hpp). Adjust if you add more
...
if(NOT report_result EQUAL 0)
  message(WARNING "llvm-cov report failed (exit ${report_result}):\n${report_error}")
endif()
...
if(NOT lines_result EQUAL 0)
  message(WARNING "llvm-cov show (lines) failed (exit ${lines_result}):\n${lines_error}")
else()
...
if(NOT html_result EQUAL 0)
  message(WARNING "llvm-cov show (HTML) failed (exit ${html_result}):\n${html_error}")
else()
